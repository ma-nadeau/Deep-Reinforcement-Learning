...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        batch = random.sample(self.buffer, batch_size)  # sample a batch
        state, action, reward, next_state, done = zip(*batch)  # unzip the batch
        # return the batch converted to numpy arrays
        return (
...
class ExpectedSarsaAgent:
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        target = reward + (1 - done) * self.gamma * expected_q
    ...
    def batch_update(self, s_batch, a_batch, r_batch, ns_batch, d_batch):
        ...
        expected_q = (
            1 - self.epsilon
        ) * max_next_q + self.epsilon * next_q_values.mean(dim=1)
...
def train(
    ...
    for trial in tqdm(range(trials), desc="Trials", leave=False):
        ...
        if agent_class == ExpectedSarsaAgent:
            filename = f"{env_name}_{'replay' if use_replay else 'no_replay'}_eps{epsilon}_lr{lr}_expected_sarsa"
        elif agent_class == QLearningAgent:
            filename = f"{env_name}_{'replay' if use_replay else 'no_replay'}_eps{epsilon}_lr{lr}_q_learning"
        with open(f"total_rewards_{filename}.txt", "a") as f:
            f.write(
                f"Trial: {trial+1}, Episode: {episode+1}, Total Reward:\n\n {all_rewards}\n"
            )
...
def plot_results(results_q, results_esarsa, env_name, use_replay, epsilon, lr):
    ...
    linestyle = "-" if lr == 0.01 else "--" if lr == 0.001 else ":"
    plt.plot(
        q_mean,
...
#TODO: Add the code for the Question 3 here
